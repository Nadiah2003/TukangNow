...
function bindProfileButtons() {
    ...
    const btnOpenMaterialModal = getEl("btnOpenMaterialModal");
    const btnCancelMaterial = getEl("btnCancelMaterial");
    const btnRefreshMaterial = getEl("btnRefreshMaterial");
    ...
    if (btnOpenMaterialModal) {
        btnOpenMaterialModal.addEventListener("click", function() {
            openModal("materialModal");
            loadBookingMaterials();
        });
    }
    ...
    if (btnRefreshMaterial) {
        btnRefreshMaterial.addEventListener("click", function() {
            loadBookingMaterials();
        });
    }
}
...
function loadBookingMaterials() {
    const tbody = getEl("materialTableBody");

    if (!tbody) {
        return;
    }

    tbody.innerHTML = `<tr><td colspan="7" class="material-empty">Loading material receipts...</td></tr>`;

    fetch(`${baseURL}/ProfileAdminServlet?action=bookingMaterials&_=${Date.now()}`, {
        method: "GET",
        credentials: "same-origin",
        cache: "no-store"
    })
    ...
    .then(data => {
        if (data.status !== "success") {
            tbody.innerHTML = `<tr><td colspan="7" class="material-empty">No data found.</td></tr>`;
            showStatus("Failed!", data.message || "Failed to load booking material receipts.", "error");
            return;
        }

        allBookingMaterials = data.materials || [];
        renderMaterialTable(allBookingMaterials);
    })
    .catch(() => {
        tbody.innerHTML = `<tr><td colspan="7" class="material-empty">Connection error.</td></tr>`;
        showStatus("Failed!", "Connection error while loading booking material receipts.", "error");
    });
}
...
function renderMaterialTable(materials) {
    ...
    if (!materials || materials.length === 0) {
        tbody.innerHTML = `<tr><td colspan="7" class="material-empty">No booking material receipt found.</td></tr>`;
        return;
    }

    tbody.innerHTML = materials.map(item => {
        const bookingId = Number(item.bookingId || 0);
        const receiptId = Number(item.receiptId || 0);
        const receiptLabel = item.receiptLabel || `Receipt ${receiptId}`;
        const itemName = item.itemName || "-";
        const quantity = item.quantity || "-";
        const price = Number(item.price || 0);
        const createdAt = item.createdAt || "-";
        const receiptPath = item.receiptPath || "";
        const receiptUrl = resolveReceiptPath(receiptPath);

        let receiptHtml = `<span style="color:#999;">No receipt</span>`;

        if (receiptUrl !== "#") {
            if (isImageReceipt(receiptPath)) {
                receiptHtml = `
                    <a href="${escapeHtml(receiptUrl)}" target="_blank">
                        <img src="${escapeHtml(receiptUrl)}" class="material-receipt-img" alt="Receipt" onerror="this.style.display='none';">
                    </a>
                    <a href="${escapeHtml(receiptUrl)}" target="_blank" class="material-receipt-link">View Receipt</a>
                `;
            } else {
                receiptHtml = `
                    <a href="${escapeHtml(receiptUrl)}" target="_blank" class="material-receipt-link">View PDF Receipt</a>
                `;
            }
        }
        ...
        return `
            <tr>
                <td><span class="material-booking-id">#${bookingId}</span></td>
                <td>${escapeHtml(receiptLabel)}</td>
                <td>${escapeHtml(itemName)}</td>
                <td>${escapeHtml(quantity)}</td>
                <td><span class="material-price">RM ${price.toFixed(2)}</span></td>
...
function resolveReceiptPath(path) {
    ...
    return `${baseURL}/ProfileAdminServlet?action=receiptImage&file=${encodeURIComponent(fileName)}&_=${Date.now()}`;
}

function isImageReceipt(path) {
    const fileName = getOnlyFileName(path).toLowerCase();
